...
public class ModelOfDiary extends Observable
{
    ...
    private int year = 2017, month = 4, day = 30;
    // private int year = 2000, month = 01, day = 01;
    ...
    /*
     *  Personally added this method to construct the key from the given date
     */
    private String constructKey(){
        String key = Integer.toString(year)+"/";
        if (month<=9){
            key = key+"0"+Integer.toString(month)+"/";
        }
        else{
            key = key+Integer.toString(month)+"/";
        }
        if (day<=9){
    ...
    public int maxDaysInMonth( int year, int month )
    {
        ...
        if (month!=2){ //If the month isnt february
            return maxDaysInMonth[month-1];
        }
        else{ // Therefore month is february
            if (year % 4 == 0){ // Checks to see if the year is a divisable by 4
